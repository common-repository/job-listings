 ...
 * @since 0.1.0
 */
class JLT_Meta_Boxes_Helper {
	public $prefix  = 'job-listings';
	public $page  = 'post';
	...
	public function add_meta_box( $meta_box, $template = '', $template_data = array() ) {
		...
		if ( !isset( $meta_box['fields'] ) || empty( $meta_box['fields'] ) ) {
			...
			case 'data-link':
				$meta_box['fields'][] = array(
					...
					'label' => __( 'The Link' , 'job-listings' ),
					'type' => 'text',
			...
			case 'data-video':
				$meta_box['fields'][] = array(
					...
					'label' => __( 'M4V File URL', 'job-listings' ),
					'desc' => __( 'Place the URL to your .m4v video file here.', 'job-listings' ),
					'type' => 'text',
				);
				$meta_box['fields'][] = array(
					'id' => $this->prefix . '_video_ogv',
					'label' => __( 'OGV File URL', 'job-listings' ),
					'desc' => __( 'Place the URL to your .ogv video file here.', 'job-listings' ),
					'type' => 'text',
				);
				$meta_box['fields'][] = array(
				...
				$meta_box['fields'][] = array(
					...
					'desc' => __( 'If you are using something other than self hosted video (YouTube, Vimeo, etc.) paste the embed code here. This field will override the above settings.', 'job-listings' ),
					'type' => 'textarea',
					'std'  => ''
				);
				break;
			...
			case 'data-image':
				$meta_box['fields'][] = array(
					...
					'label' => __( 'Upload Image', 'job-listings' ),
					'type' => 'image',
				);
				break;
		...
		$callback = create_function( '$post,$meta_box', 'jlt_create_meta_box( $post, $meta_box["args"] );' );
