...
class JLT_Widget extends WP_Widget {
...
class JLT_Job_Search_Widget extends JLT_Widget {
	public function __construct() {
		$this->widget_cssclass    = 'jlt-job-search-widget';
		$this->widget_description = __( "Search form for Jobs.", 'job-listings' );
		$this->widget_id          = 'jlt_job_search_widget';
		$this->widget_name        = __( 'Job Listings Job Search', 'job-listings' );
		$this->cached             = true;
		parent::__construct();
	}
	...
	public function get_fields() {
		if ( empty( $this->fields ) ) {
			$this->fields = array(
				'title' => array( 'type' => 'text', 'std' => '', 'label' => __( 'Title', 'job-listings' ) ),
			);
		}
...
function jlt_register_widget() {
	if ( class_exists( 'JLT_Job' ) ) {
		register_widget( 'JLT_Job_Search_Widget' );
	}
}

add_action( 'widgets_init', 'jlt_register_widget' );